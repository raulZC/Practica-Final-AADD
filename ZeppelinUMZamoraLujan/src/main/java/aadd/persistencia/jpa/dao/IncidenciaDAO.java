...
public class IncidenciaDAO extends ExtensionDAO<Incidencia> {
	...
	public List<IncidenciaDTO> transformarToDTO(List<Incidencia> incidencias) {
		...
		for (Incidencia i : incidencias) {
			inc.add(new IncidenciaDTO(i.getId(), i.getFechaCreacion(), i.getDescripcion(), i.getFechaCierre(),
					i.getComentarioCierre(), i.getUsuario(), i.getRestaurante(),i.getPedido()));
		}
		return inc;
	}
	...
	public List<IncidenciaDTO> findByUsuario(Integer idUsuario) {
		...
		try {
			final String queryString = " SELECT incidencia from " + name
					+ " incidencia WHERE incidencia.usuario.id = :idUsuario ";
	...
	public List<IncidenciaDTO> findNoCerradas(Integer idRestaurante) {
		...
		try {
			Query query = EntityManagerHelper.getEntityManager().createNamedQuery("Incidencia.findNoCerradasRest");
	...
	public List<IncidenciaDTO> findByRestaurante(Integer idRestaurante) {

		try {
			Query query = EntityManagerHelper.getEntityManager().createNamedQuery("Incidencia.findIncidenciasRest");
			query.setParameter("idRestaurante", idRestaurante);
			return transformarToDTO(query.getResultList());
		} catch (RuntimeException re) {
			throw re;
