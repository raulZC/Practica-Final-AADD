...
public class ServicioGestionPedido {
	...
	public TipoEstado avanzarEstado(ObjectId id) {
		...
		case PREPARADO:
			...
			zeppelinumRemoto.pedidoRecogido(id.toString());
	...
	public Integer crearIncidencia(String descripcion, Integer idUsuario, Integer idRestaurante, ObjectId idPedido) {
	    ...
	    try {
	        ...
	        if (usuario == null){
	            ...
	            throw new Exception("No se encontr� el usuario con ID " + idUsuario);
	        }
	        // Buscar el restaurante
	        Restaurante restaurante = RestauranteDAO.getRestauranteDAO().findById(idRestaurante);
	        if (restaurante == null){
	            // Si no se encuentra el restaurante, lanzar una excepci�n
	            throw new Exception("No se encontr� el restaurante con ID " + idRestaurante);
	        }
	...
	public Integer cerrarIncidencia(Integer idIncidencia,String comentarioCierre) {
	    ...
	    try {
	      ...
	      //Buscar incidencia
	        ...
	        if (incidencia == null){
	            // Si no se encuentra el usuario, lanzar una excepci�n
	            throw new Exception("No se encontr� la incidencia con ID " + idIncidencia);
	        }
	        ...
	        if (usuario == null){
	            // Si no se encuentra el usuario, lanzar una excepci�n
	            throw new Exception("No se encontr� el usuario con ID " + incidencia.getUsuario().getId());
	        }
	        // Buscar el restaurante
	        Restaurante restaurante = RestauranteDAO.getRestauranteDAO().findById(incidencia.getRestaurante().getId());
	        if (restaurante == null){
	            // Si no se encuentra el restaurante, lanzar una excepci�n
	            throw new Exception("No se encontr� el restaurante con ID " + incidencia.getRestaurante().getId());
	        }
