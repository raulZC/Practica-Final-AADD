...
public class ServicioGestionPlataforma {
	...
	public Plato getPlatoByid(Integer id) {
		return PlatoDAO.getPlatoDAO().findPlatoById(id);
	}
	...
	public RestauranteDTO getRestaurante(Integer idRestaurante) {
		...
		return new RestauranteDTO(idRestaurante, restaurante.getNombre(), restaurante.getValoracionGlobal(),restaurante.getResponsable().getId());
	}
	...
	public List<IncidenciaDTO> getIncidenciasByUsuario(Integer idUsuario){
		...
		return IncidenciaDAO.getIncidenciaDAO().findByUsuario(idUsuario);
	}
